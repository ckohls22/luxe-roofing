...
export default function HomePage() {
  ...
  const [selectedPolygonIndex, setSelectedPolygonIndex] = useState<
    number | null
  >(null); // Index of the currently selected polygon for editing
  ...
  // handle confirm roof selection ( see my price button )
  const handleConfirmRoofSelection = useCallback(() => {
    if (roofPolygons.length === 0) {
      setError("Please draw at least one roof polygon before proceeding.");
      return;
    }
    // Proceed to the next step, e.g., show pricing or quote form
    // console.log("Confirmed roof selection:", roofPolygons);
  ...
  const sampleRoofPolygons = [
    {
      ...
      area: {
        squareMeters: 100,
        squareFeet: 1076,
        formatted: "100 sq m / 1076 sq ft"
      },
      label: "Main Roof",
      centerPoint: [40.7133, -74.0055], // Center point
      slope: "5/12" // Example slope
    },
  ];
    ...
    <div>
      <LeadForm
          ...
          initialData={{
            ...
            selectedRoofPolygons: sampleRoofPolygons // Pass roof data from your map component
          }}
          className="form-card"
        />
      ...
      {/* main container */}
      { currentStep !== 'quote' && (searchFocus || selectedAddress) && (
        <div className="flex sm:flex-col gap-4  border border-black rounded-4xl  shadow-lg w-[800px] h-[500px] p-5 mx-auto  box-border overflow-hidden ">
          ...
          {selectedAddress && currentStep === "drawing" && (
            <div className="h-full max-w-[340px] min-w-[330px]  rounded-2xl border-2  relative overflow-hidden">
              ...
              <div className="absolute left-0 right-0 bottom-0 z-50 text-white p-4 rounded-b-2xl shadow-lg backdrop-blur-2xl">
                {/* <p>Total Roof Area: 345 sq ft</p> */}
                {/* <Button onClick={handleRoofEditable}>Edit Roofs</Button> */}
                <Button className="w-full bg-orange-500" onClick={handleConfirmRoofSelection}>See My Price</Button>
              </div>
            </div>
          )}
          {/* {currentStep === "quote" && (
      ...
      { currentStep === "quote" && (
        <div className="flex gap-4  border border-black rounded-4xl  shadow-lg w-[800px] h-[500px] p-5 mx-auto  box-border overflow-hidden">
          ...
          <Button className=" " onClick={() => setCurrentStep("drawing")}> ⬅ Back</Button>
          <LeadForm/>
          </div>
      )}
      {/* Center Column - Map */}
