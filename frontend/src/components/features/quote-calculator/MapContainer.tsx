...
import mapboxgl, { Map as MapboxMap, MapSourceDataEvent } from "mapbox-gl";
...
} from "@/lib/mapbox/drawing";
import {
  detectBuildingAtLocation,
  fitMapToBuilding,
} from "@/lib/mapbox/building-detection";
...
export function MapContainer({
   ...
   */
  const updateRoofCalculations = useCallback(() => {
    ...
    const roofPolygons: RoofPolygon[] = features.map((feature, index) => {
      ...
      const centerCoords = centerOfMass(polygon).geometry.coordinates as [
        number,
        number
      ];
  ...
  [mapRef, onLoadingChange, updateRoofCalculations]
);
  ...
  }, [mapRef, isLoaded, updateRoofCalculations]);

  // Handle address selection and building detection
  useEffect(() => {
    if (!mapRef.current || !selectedAddress || !drawRef.current || !isLoaded)
      return;
    console.log("Address effect triggered:", selectedAddress);
    detectAndDrawBuilding(selectedAddress.coordinates);
  }, [mapRef, isLoaded, selectedAddress?.coordinates, detectAndDrawBuilding, selectedAddress]);
  ...
  //     selectedPolygonIndex >= roofPolygons.length
  //   )
  //     return;
  ...
  //     (f) => String(f.id) === String(selectedRoof.id)
  //   );
  ...
  //   features.forEach((f) => {
  //     if (f.id !== undefined)
  //       draw.setFeatureProperty(String(f.id), "user_color", "gray");
  //   });
