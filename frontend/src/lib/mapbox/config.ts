...
import { MapboxConfig } from "@/types";
...
export const initializeMapbox = (): void => {
  if (!mapboxConfig.accessToken) {
    console.error(
      "Mapbox access token is missing. Please set NEXT_PUBLIC_MAPBOX_TOKEN environment variable."
    );
    return;
  }
  ...
  console.log(
    ...
    mapboxConfig.accessToken ? "YES" : "NO"
  );
};
...
export const createMapOptions = (
  ...
  customOptions: Partial<mapboxgl.MapOptions> = {}
): mapboxgl.MapOptions => {
  const opts = {
    container: container,
